...
export async function submitTeacherAttendance(formData: FormData) {
  ...
  if (!assignment) throw new Error('Not authorized for this group');
...
export async function saveTeacherLessonTopic(formData: FormData) {
  ...
  const { data: assignment } = await db
    .from('teacher_groups').select('teacher_id')
    .eq('teacher_id', session.id).eq('group_id', group_id).maybeSingle();
